...
Option `--list-files` only creates a file containing the names of the targeted files instead of downloading them.
You have to input a file name: `--list_files my_files.txt`. The format needs to be `.txt` or `.csv`:
- if the user inputs a file name that ends in `.txt` then this file contains only the full s3 path to the targeted files and is compatible with the `--file-list` option. See example of `my_files.txt`:
```
...
```
> **_NOTE:_**  This option is compatible with the file generated by the `--list-files` option if you generated a ".txt" file.
