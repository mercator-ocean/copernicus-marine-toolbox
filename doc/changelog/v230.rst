...
Especially, it introduces a new ``split-on`` subcommand for the ``subset`` command, allowing users to split output files based on time or variables.
...
* Improved the documentation page *Polygons Extractor Using the Copernicus Marine Toolbox* in the *Usage and Technical Details* section. It is now clearer and requires fewer dependencies. See Polygons Extractor.
...
* Updated the output of DatasetVersionNotFound to be more descriptive.
* Some minor documentation fixes and improvements.
* Fixed an issue where using ``request-file`` option would not work properly with boolean options.
...
* Added a new subcommand ``split-on`` to the ``subset`` command and a new ``subset_split_on`` Python function to split the output files based on time or variables when subsetting. See :ref:`subset usage page <subset-split-on>` for more details.
...
* The argument ``configuration_file_directory`` in the ``login`` function can now accept both string and pathlib.Path types as prescribed in the documentation.
