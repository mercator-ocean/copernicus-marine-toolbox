...
* Added a way to use the Toolbox without threads by setting the environment variable ``COPERNICUSMARINE_USE_THREADS`` to "False". See :ref:`documentation about environment variables <env-use-threads>` for more details.
* Improved the display of sizes in the output of the get and subset commands by using human-readable format and limiting the number of decimal places to two.
* Release constraints on ``pandas``. ``pandas>=3.0.0`` is now compatible with the Toolbox.
* Release constraints on ``click``. ``click>=8.3.2`` can now be used with the Toolbox.

Fixes
^^^^^

* The CLI now raises an error if an argument is provided twice instead of using the last provided value.
* Some minor documentation improvements and updates.
...
* Added support for subsetting sparse datasets in NetCDF format. See the :ref:`documentation about the new NetCDF format <sparse-netcdf>` for more details.
...
* Handle chunks parts for sparse datasets. Resulting subsets should be more complete.
...
* Login will now reuse existing credentials if they are still valid, instead of prompting the user to log in again.
