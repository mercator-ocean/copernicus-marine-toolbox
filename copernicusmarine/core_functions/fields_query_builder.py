...
class QueryBuilder:
    ...
    def _return_available_fields(
        self, type_to_check: Type, available_fields: Optional[set[str]] = None
    ) -> set[str]:
        """
        ...
        for (
            ...
            for base_model, _ in (all_base_models or {}).items():
                result = self._return_available_fields(
                    base_model, available_fields
                )
                available_fields.union(result)
        return available_fields

    def get_queryable_requested_fields(
        self, requested_fields: set[str], type_to_check: Type, option_name: str
    ) -> set[str]:
        """
        ...
        """  # noqa: E501
        available_fields = self._return_available_fields(type_to_check)
